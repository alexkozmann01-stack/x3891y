...
namespace optim
{
    ...
    {
        m_backups.Load();
        m_catalog = BuildCatalog(&m_backups);
    ...
    void Service::ApplyAsync(const std::string& id)
    {
        ...
        m_worker->Enqueue([this, target, index]() {
            ...
            m_backups.RecordHistory(target->info().id, "apply", error.ok(),
                error.ok() ? "Použité a overené." : error.message);
            m_backups.Save();
            ...
            result.index = index;
            result.status = status;
            result.outcome = Outcome{ target->info().id, "apply", error.ok(),
                error.ok() ? "Použité a overené." : error.message };
            m_pending.push_back(result);
